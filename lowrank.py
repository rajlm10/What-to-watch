...
Raj,Rocks=gradient_descent(X,Theta,Y,R,10000,6*1e-4)
...
for s in range(9719):
    if Raj[s]>=5:
        RR.append(columns[s]+str(Raj[s]))
    

testing=Y.T[600]
testing1=Ypredict[600]   

extra=[]
for e in range(len(columns)):
    if columns[e]==('Dark Knight, The (2008)'):
        extra.append(str(Sanaya[e])+' '+columns[e])
